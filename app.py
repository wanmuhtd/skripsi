...
# ======================================================
# 2. MODERN DASHBOARD CSS (PERBAIKAN KOTAK)
# ======================================================
st.markdown("""
<style>
    ...
    /* Mengatur gaya container asli Streamlit agar menjadi Card */
    ...
    /* Menghilangkan border pada kolom agar tidak double kotak */
    ...
    .res-label { font-size: 0.9rem; color: #8b949e; }
...
# 4. HEADER
# ======================================================
st.markdown("<h1 style='text-align: center; font-weight: 800; margin-top: 0;'>🩺 Diabetes Intelligence</h1>", unsafe_allow_html=True)
...
tab1, tab2 = st.tabs(["Analyze Patient", "Methodology"])

with tab1:
    col_left, col_right = st.columns([1, 1.3], gap="large")

    with col_left:
        # Menggunakan st.container(border=True) sebagai ganti div manual
    ...
    with col_right:
        if predict_btn:
            ...
            with st.container(border=True):
                st.markdown("<div style='min-height: 410px; display: flex; align-items: center; justify-content: center; color: #484f58;'>Result will appear after analysis.</div>", unsafe_allow_html=True)

with tab2:
    with st.container(border=True):
        st.markdown("### Methodology Overview")
        st.write("Sistem ini menggunakan Denoising Autoencoder (DAE) untuk kompensasi fitur dan Stacking Ensemble untuk klasifikasi akhir.")
